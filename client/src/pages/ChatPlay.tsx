...
export default function ChatPlay() {
  ...
  const [cleanupStage, setCleanupStage] = useState<"idle" | "working">("idle");
  ...
  useEffect(() => {
    ...
    const handleOffline = () => { setIsOnline(false); setRealtimeStatus("offline"); };
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`room:${roomId}`, { config: { private: true, presence: { key: profile?.id ?? "guest" } } })
      ...
      .subscribe(async status => {
        if (status === "SUBSCRIBED") { setRealtimeStatus("connected"); if (profile) await channel.track({ profile }); }
        else if (status === "CHANNEL_ERROR" || status === "TIMED_OUT" || status === "CLOSED") setRealtimeStatus(navigator.onLine ? "reconnecting" : "offline");
      });
    return () => { void supabase.removeChannel(channel); };
  ...
  useEffect(() => {
    ...
    if (!viewport) return;
    if (messages.length === 0 || shouldStickToBottom(viewport.scrollHeight, viewport.scrollTop, viewport.clientHeight)) {
